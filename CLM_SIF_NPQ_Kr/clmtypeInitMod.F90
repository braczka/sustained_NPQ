...
#endif
     allocate(pps%stomatal_cond(beg:end)) ! BMR 8/6/15
...
#endif
    pps%stomatal_cond(beg:end) = nan      !  BMR 8/6/15
   ! and vertical profiles for calculating fluxes
...
!------------------------------------------------------------------------
    ...
    allocate(pcs%cistar(beg:end))	! BMR added 5/16/15
    if ( crop_prog )then
    ...
    pcs%cistar(beg:end) = nan		! BMR added 5/16/15
    if ( crop_prog )then
...
!lje & XY****
    allocate(pcf%sifsun(beg:end))
    allocate(pcf%sifsha(beg:end))
    allocate(pcf%fsif(beg:end))
    allocate(pcf%sifsun_z(beg:end,1:nlevcan))
    allocate(pcf%sifsha_z(beg:end,1:nlevcan))
!lje & XY$$$$
    allocate(pcf%fyield(beg:end))               ! BMR
    allocate(pcf%pyield(beg:end))               ! BMR
    allocate(pcf%fyieldsun(beg:end))            ! BMR
    allocate(pcf%pyieldsun(beg:end))            ! BMR
    allocate(pcf%fyieldsha(beg:end))            ! BMR
    allocate(pcf%pyieldsha(beg:end))            ! BMR
    allocate(pcf%fxsat(beg:end))                ! BMR
    allocate(pcf%xsatsun(beg:end))              ! BMR
    allocate(pcf%xsatsha(beg:end))              ! BMR
    allocate(pcf%fxrev(beg:end))               ! BMR
    allocate(pcf%xrevsun(beg:end))               ! BMR
    allocate(pcf%xrevsha(beg:end))               ! BMR
    ...
    pcf%ansun(beg:end) = nan
    pcf%ansha(beg:end) = nan
...
!lje & XY****
    pcf%sifsun(beg:end) = nan
    pcf%sifsha(beg:end) = nan
    pcf%fsif(beg:end) = spval
    pcf%sifsun_z(beg:end,1:nlevcan) = nan
    pcf%sifsha_z(beg:end,1:nlevcan) = nan
!lje & XY$$$$
    pcf%fyield(beg:end) = spval      ! BMR
    pcf%pyield(beg:end) = spval      ! BMR
    
    pcf%fyieldsun(beg:end) = nan      ! BMR
    pcf%fyieldsha(beg:end) = nan      ! BMR  
    pcf%pyieldsun(beg:end) = nan      ! BMR
    pcf%pyieldsha(beg:end) = nan      ! BMR
    pcf%fxsat(beg:end) = spval      ! BMR
    pcf%xsatsun(beg:end) = nan      ! BMR
    pcf%xsatsha(beg:end) = nan      ! BMR
...
!   allocate(cps%xirrig(beg:end))
    ...
    allocate(cps%lh(beg:end))
    allocate(cps%ig(beg:end))
...
!   cps%xirrig(beg:end) = 0._r8
    ...
    cps%lh(beg:end) = spval
    cps%ig(beg:end) = spval
