...
!
   ...
   real(r8), pointer :: ansun(:)           ! sunlit leaf net assimilation (umol CO2 /m**2/ s) BMR  8/6/15
   ...
   real(r8), pointer :: ansha(:)           ! shaded leaf net assimilation (umol CO2 /m**2/ s) BMR 8/6/15
!lje & BMR ****
    real(r8), pointer :: sifsun(:)          ! sunlit leaf solar-induced fluorescence per unit leaf area (W /m**2)  
    real(r8), pointer :: sifsha(:)          ! shaded leaf solar-induced fluorescence per unit leaf area (W /m**2)
!lje & BMR $$$$
!bmr$$$$
    real(r8), pointer :: fyieldsun(:)          ! sunlit fluorescence yield (photon absorbed photon emitted-1 for PAR), diagnostic variable
    real(r8), pointer :: fyieldsha(:)          ! shaded fluorescence yield (photon absorbed photon emitted-1 for PAR), diagnostic variable
    
    real(r8), pointer :: pyieldsun(:)          ! sunlit photochemical yield (photon absorbed photon emitted-1 for PAR), diagnostic variable
    real(r8), pointer :: pyieldsha(:)          ! shaded photochemical yield (photon absorbed photon emitted-1 for PAR), diagnostic variable
    
    real(r8), pointer :: xsatsun(:)          ! sunlit ratio of actual/potential (ja/qabs) electron transport for fluorescence model
    real(r8), pointer :: xsatsha(:)          ! shaded ratio of actual/potential (ja/qabs) electron transport for fluorescence model
    
    real(r8), pointer :: xrevsun(:)          ! sunlit degree of reversible light saturation  (ja/je)
    real(r8), pointer :: xrevsha(:)          ! shaded degree of reversible light saturation  (ja/je)
...
!lje & bmr****
    real(r8), pointer :: fsif(:)           ! solar-induced fluorescence per unit ground area (W /m**2)
    real(r8), pointer :: fsa(:)            ! solar radiation absorbed (total) 
!bmr$$$$
    real(r8), pointer :: fyield(:)           ! fluorescence yield, canopy weighted diagnostic (photon absorbed photon emitted-1 of PAR)
    real(r8), pointer :: pyield(:)           ! photochemical yield canopy weighted diagnostic (photon absorbed photon emitted-1 of PAR)
    real(r8), pointer :: fxsat(:)           ! ratio of actual/potential (jab/qabs) electron transport for fluorescence model, canopy weighted 
    real(r8), pointer :: fxrev(:)           ! ratio of actual/potential (jab/je) electron transport for fluorescence model, canopy weighted 
!bmr$$$$
...
#endif
   real(r8), pointer :: stomatal_cond(:)       ! fpsn weighted stomatal resistance [m/s], BMR 9/4/15
...
!
   ...
   integer, save :: c13_init_counter = 0      ! counting variable, using save to make permanent   BMR edit
...
!lje & bmr$$$$
!bmr$$$$
    fyieldsun         => pcf%fyieldsun
    fyieldsha         => pcf%fyieldsha
    pyieldsun         => pcf%pyieldsun
    pyieldsha         => pcf%pyieldsha
    xsatsun           => pcf%xsatsun
    xsatsha           => pcf%xsatsha
    xrevsun           => pcf%xrevsun
    xrevsha           => pcf%xrevsha    
...
#endif
   stomatal_cond    => pps%stomatal_cond     ! BMR 8/6/15
...
if (elai(p)>0) then
      ...
      fxrev(p) =  (laisun(p)*xrevsun(p) +  laisha(p)*xrevsha(p))/ max(elai(p), 0.01_r8)
else
fyield(p)=0._r8
pyield(p)=0._r8
fxsat(p)=0._r8
...
!bmr*****
      ...
      ! Calculate fpsn weighted stomatal resistance  BMR 9/4/15
      ! This did not seem to work.....went back to traditional lai weighting
      ! stomatal_cond(p) = (laisun(p)*psnsun(p)*(rssun(p)) + laisha(p)*psnsha(p)*(rssha(p)))/max(fpsn(p), 0.01_r8)

      ! Calculate lai weighted stomatal resistance BMR 11/10/15

        stomatal_cond(p) = (laisun(p)*rssun(p) + laisha(p)*rssha(p))/max(elai(p), 0.01_r8)
...
! !USES:
   ...
   use clm_time_manager , only : get_step_size, get_curr_calday       ! BMR 7/29/15 gpp_downscale edit v3
#if (defined CN)
...
! bmr
   real(r8) :: ci_z_downreg(lbp:ubp,nlevcan)   ! downregulated intracellular leaf CO2 (Pa)
! bmr   
   real(r8), pointer :: an_z(:,:)   ! canopy layer: net assimilation (umol co2 /m**2/ s) [always +], BMR 8/6/15
   ...
   real(r8), pointer :: anet(:)      ! foliage net assimilation (umol co2 /m**2/ s) [always +]      BMR 8/6/15
...
!lje****
   real(r8), pointer :: sif(:)      ! foliage solar-induced fluorescence (W /m**2) [always +]    
   real(r8), pointer :: sif_z(:,:)  ! canopy layer: solar-induced fluorescence (W /m**2/) [always +]  
...
!bmr***     
    ...
    real(r8),pointer :: xsat(:)  ! ratio of actual/potential electron transport for fluorescence model (ja/qabs)
    real(r8),pointer :: xrev(:)  ! ratio of actual/potential electron transport for fluorescence model (ja/je)
!bmr***

!bmr 
     real(r8), pointer :: forc_pco2(:)    ! partial pressure co2 (Pa)
     real(r8) :: co2(lbp:ubp)   ! partial pressure co2 (Pa)
...
!bmr***
   real(r8) :: fs_z(lbp:ubp,nlevcan)  ! canopy layer: fluorescence yield   
   real(r8) :: ps_z(lbp:ubp,nlevcan)  ! canopy layer: photochemical yield 
   real(r8) :: xsat_z(lbp:ubp,nlevcan)  ! canopy layer: ratio of actual/potential (ja/qabs) electron transport for fluorescence model 
   real(r8) :: xrev_z(lbp:ubp,nlevcan)  ! canopy layer: ratio of actual/potential (ja/je) electron transport for fluorescence model 
!bmr***
...
!lje & XY & bmr**** general for shaded or sunlit 
   ...
   real(r8) :: xrevcan            ! canopy sum of xrev_z
...
!KO   tpuc   = fth25 (tpuhd, tpuse)
   ...
   ! BMR 3/29/17  Using gpp_downreg from original_gppdownscale (best fit from Raczka et al (2016))
   !  0.085 scalar includes weakening factor adjustment
   
         ! Calculate the downscaling term as function of julianday apply to vcmaxtop
         julday=get_curr_calday(0)      ! offset=0
   
         if (julday >= 83.0_r8 .and. julday <= 316.0_r8) then
         
         ! Growing season downregulation

            gpp_downreg=(1.09E-12)*julday**6+ (-1.351302E-9)*julday**5+ &
                     (6.72179301E-7)*julday**4+ (-1.70891141427E-4)*julday**3+ &
                     (2.3239698420533E-2)*julday**2+ (-1.583596002259290)*julday+ &
                     42.219833669+0.085

            if (gpp_downreg < 0.0_r8) then
             
               gpp_downreg= 0.0001_r8
            endif


         ! To turn off growing season downregulation only, just set to 1

         !  gpp_downreg=1.0_r8

         ! Growing season downregulation end
                      
         else
       
         ! Winter time regulation
         gpp_downreg=0.0001_r8        ! GPP=0 during the winter
   ...
   do f = 1, fn
      ...
      ! vcmax25 at canopy top, as in CN but using lnc at top of the canopy
           ...
           !write (iulog,*) 'current julday value:'
           !write (iulog,*) julday
           !write (iulog,*) 'current gpp_downreg value:'
           !write (iulog,*) gpp_downreg

          vcmax25top = lnc(p) * flnr(ivt(p)) * fnr * act25 * dayl_factor(p) * gpp_downreg
        ! vcmax25top = lnc(p) * flnr(ivt(p)) * fnr * act25 * dayl_factor(p)    ! Turns off winter and growing season downregulation

! ! End BMR 7/29/15 gpp_downreg mods version 3
...
!KO
            ...
            psn_z(p,iv) = ag(p,iv)              !(umol CO2/m**2/s)
            an_z(p,iv) = ag(p,iv)-lmr_z(p,iv)   ! BMR 8/6/15
            psn_wc_z(p,iv) = 0._r8
            psn_wj_z(p,iv) = 0._r8
            psn_wp_z(p,iv) = 0._r8
...
!lje & XY & BMR****
	...
	if(c3flag(p)) then
	   ...
	   ps_z(p,iv)=psn_z(p,iv)/aj(p,iv)*po0
	    ...
	    ! BMR 2/28/19 light saturation (x) edit
	    
	    
	    call fluorescence(ps_z(p,iv),po0,ja,par_z(p,iv),fs_z(p,iv),xrev_z(p,iv)) 
	    
	    if (psn_z(p,iv) <= 0._r8)  xrev_z(p,iv)=1._r8          
...
!lje & XY & BMR****
	 ...
	 xrevcan = xrevcan + xrev_z(p,iv) * lai_z(p,iv)
!lje & XY & BMR$$$$
      ...
      if (laican > 0._r8) then
         psn(p) = psncan / laican
!lje & XY & BMR****  Per leaf area basis for sunlit or shaded layer
         sif(p) = sifcan / laican		    
	 fs(p)=fyieldcan / laican
	 ps(p)=pyieldcan / laican
         xsat(p)=xsatcan / laican
	 xrev(p)=xrevcan / laican	 	 
!lje & XY & BMR$$$$
...
!
! !INTERFACE:
subroutine fluorescence(ps,po0,ja,par,fs,x_reversible)
   ...
   ! 2/28/19 Raczka light saturation modifications
   !!USES
   use shr_kind_mod , only : r8 => shr_kind_r8
   !
   !ARGUMENTS:
   ...
   real(r8), intent(in) :: ja   ! actual electron transport
   real(r8), intent(in) :: par  ! photochemical active radiation
   real(r8), intent(out) :: fs  ! fluorescence yield
   real(r8), intent(out) :: x_reversible   ! degree of light saturation, phip_night 
   ...
   real(r8) :: Kp       ! rate constant for photochemical quenching
   real(r8) :: Ks       ! rate constant for sustained non-photochemical quenching
   real(r8) :: x        ! degree of light saturation
   real(r8) :: fm       ! light adapted fluorescence yield Fm
   
   real(r8) :: phip_night     ! dark-adapted photochemical efficiency,
                              ! acccounts for sustained NPQ
   real(r8) :: je_reversible  ! potential electron transport, phip_night
  ...
  !Kp   = 4.0_r8             ! Fixed Kp
   ...
   x    = 1._r8 - ps / po0   ! Deprecated version of x (applicable for summer when Ks~0), dummy variable now
   
   call NPQ(x,Kn,Ks)        ! Call NPQ subroutine to get kNPQs (Ks) 
                            ! Function of time of year only
   
   Kp   = 4.0_r8*(1 - Ks/7.0_r8)             ! Downscaling Kp when Ks is high
   phip_night =Kp/(Kp+Kd+Kf+Ks)
   je_reversible  = 0.5_r8 * (phip_night) * par * 4.6_r8
   
   x_reversible  = 1._r8 - ja / je_reversible    ! 2/28/19 Updated version used to calculate NPQ 
                                                 ! Accounts for seasonal-varying Ks
   x_reversible=max(x_reversible, 0._r8)        
   
   
   call NPQ(x_reversible,Kn,Ks)         ! Call NPQ subroutine to get total NPQ (sustained and reversible)
   
   
!  Kn   = (6.2473_r8 * x - 0.5944_r8) * x       ! empirical fit to Flexas' data; Used in case CLM_SIF45_v1
!  Kn   = (3.9867_r8 * x - 1.0589_r8) * x       ! empirical fit to Flexas, Daumard, Rascher, Berry data

   fm   = Kf / (Kf + Kd + Kn)
   
!   fs   = fm * (1._r8 - ps)      ! Old version of photochemical yield based on fixed phip_night (phi_max)

    if (ja > je_reversible) je_reversible = ja      ! Prevent ja > je_reversible 

    fs   = fm * (1._r8 - (ja / je_reversible)*phip_night)      ! phip_night accounts for changing Ks throughout year
   
   
    !write (iulog,*) 'phip_night'
    !write (iulog,*)  phip_night
    !write (iulog,*) 'ja -actual tranpsport '
    !write (iulog,*)  ja
    !write (iulog,*) 'je_reversible umol photons/m**2/s'
    !write (iulog,*) je_reversible
	!write (iulog,*) 'par'
        !write (iulog,*)  par  
   	!write (iulog,*) 'x_reversible'
          !write (iulog,*) x_reversible
          !write (iulog,*) 'fm'
          !write (iulog,*) fm 
          !write (iulog,*) 'fs'
          !write (iulog,*) fs 
   	  !write (iulog,*) ' '
    	  !write (iulog,*) 'end step'
          !write (iulog,*) ' '    
  
   
   
    !       Some troubleshooting
    !      write (iulog,*) 'current Kn value in fluorescence subroutine:'
    !      write (iulog,*) Kn
    !	   write (iulog,*) 'current fm value in fluorescence subroutine:'
    !      write (iulog,*) fm
    !      write (iulog,*) 'current fs value in fluorescence subroutine:'
    !      write (iulog,*) fs 
    !	   write (iulog,*) ' '
    !	   write (iulog,*) 'end step'
    !	   write (iulog,*) ' '    
  
     !     write (iulog,*) 'old version of x (light saturation)'
     !     write (iulog,*) x
     !     write (iulog,*) 'new version of x_reversible'
     !     write (iulog,*) x_reversible
     !     write (iulog,*) 'reversible NPQ (Kn)'
     !     write (iulog,*) Kn-Ks
     !	   write (iulog,*) 'sustained NPQ (Ks)'
     !     write (iulog,*) Ks
     !	   write (iulog,*) 'light-adapted fluorescence yield'
     !     write (iulog,*) fm
     !     write (iulog,*) 'fluorescence yield:'
     !     write (iulog,*) fs  
     !	   write (iulog,*) 'photochemical yield:'
     !     write (iulog,*) ps   
...
subroutine NPQ(x,Kn,kNPQs)
   ...
   ! 2/28/2019  Uses Kolari method to adjust kNPQs(time)
   ! Also uses time varying kNPQr based on synchronized transition with Kolari kNPQs
   ...
   real(r8), intent(out) :: kNPQs  ! sustained NPQ rate constant (Ks); function of time of year
   ...
   real(r8):: transition_state  ! % NPQs transition from summer to winter state
   
   ! Perform the case specific calculations here.
   ! 6/12/18 Using kNPQs coefficient for NWR temperature adjustment 
   ! Kn function updated;  kn= kNPQs(time)+kNPQr(x)
   ! 2/28/2019  Uses Kolari method to adjust kNPQs(time)
 ...
 ! Reversible, time varying,  NPQ rate coefficient
 ! kNPQr coefficients based upon summer and winter boundaries fitted from 3 parameter Van der Tol
 ! Transition timing is made to sync with Kolari method for sustained NPQ
 
if (julday>196_r8 .and. julday<=258) then
  ! set permanently at summer setting
 
    transition_state=0_r8
    
    else if (julday>15 .and. julday<=46) then
    transition_state=0.0043*julday+0.8029
  
    else if (julday>46 .and. julday<=74) then
    transition_state=-0.0035*julday+1.1595

    else if (julday>74 .and. julday<=105) then
    transition_state=-0.0074*julday+1.4474
    
    else if (julday>105 .and. julday<=135) then
    transition_state=-0.0127*julday+2.0067
    
    else if (julday>135 .and. julday<=166) then
    transition_state=-0.0078*julday+1.3426
    
    else if (julday>166 .and. julday<=196) then
    transition_state=-0.0014*julday+0.2803
    
    else if (julday>196 .and. julday<=227) then
    transition_state=-0.0003*julday+0.0685

    else if (julday>227 .and. julday<=258) then
    transition_state=0.0003*julday-0.0683
    
    else if (julday>258 .and. julday<=288) then
    transition_state=0.0029*julday-0.7405
    
    else if (julday>288 .and. julday<=319) then
    transition_state=0.0100*julday-2.7842
    
    else if (julday>319 .and. julday<=349) then
    transition_state=0.0158*julday-4.6286
    
    
   else
 
    transition_state=0.87 


 end if
 
 
  Kno=3.642-(3.642-0.6908)*transition_state
  Beta=10180-(10180-0.01417)*transition_state
  Alpha=1.413-(1.413-1.344)*transition_state
...
! !USES:
   ...
   use clm_atmlnd         , only : clm_a2l  
   use shr_kind_mod       , only : r8 => shr_kind_r8
   use clm_time_manager   , only : get_curr_calday    ! BMR 7/22/15 gpp_downscale edit
   implicit none
...
!   
   integer  :: f,p,g,iv              ! indices
   real(r8) :: co2(lbp:ubp)          ! atmospheric co2 partial pressure (pa)
!   real(r8) :: ci              ! outputting cistar for C13 fractionation  commented out BMR 5/16/15
   real(r8), pointer :: ci(:)   ! BMR added 5/16/15
   real(r8):: gpp_downreg           ! BMR 7/22/15 edit
   real(r8):: julday                ! BMR 7/22/15 edit
!------------------------------------------------------------------------------
   ...
   gb_mol         => ppsyns%gb_mol   
   gs_mol         => ppsyns%gs_mol
   ci             => pcs%cistar       ! BMR 5/16/15,  Assigning local variable, ci to global variable cistar
                                      ! Below, all former 'ci' converted to 'ci(p)' 
   if (phase == 'sun') then
      par_z       => pef%parsun_z
      alphapsn    => pps%alphapsnsun 
   else if (phase == 'sha') then
